...
const getData = ()=>{
  setLoading(true);
  axios.get('http://localhost:3000/product/category/66e0abe62e6bda2ea8fee81b')
  .then(res=>{
    setLoading(false);
...
{ (filteredProducts.length > 0 ? filteredProducts : mobile).map((eachMobile,index)  => {
  return (
    ...
    <div className='handbagItemDetails-container'>
      ...
      <div className='upd-del' style={{display:'flex',flexDirection:"column",gap:'10px'}}>
