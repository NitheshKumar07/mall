...
const getData = ()=>{
  ...
  .then(res=>{
    ...
    setdublicateColour([...new Set(res.data.product.map(product => product.colour))]); //colour filter

    setdublicateBrand([...new Set(res.data.product
    .filter(product => product.brandName) // Filter out products without a brandName
...
{ (filteredProducts.length > 0 ? filteredProducts : mobile).map((eachMobile,index)  => {
  return (
    ...
    <div className='mobileItemImg-container'>
        <div className='mobileItemImg-box'>
    ...
    <div className='handbagItemDetails-container'>
      ...
      <div className='upd-del' style={{display:'flex',flexDirection:'column',gap:'10px',width:'90%'}}>
